...
def migration_invoice_moves(env):
    ...
    openupgrade.logged_query(
        ...
        amount_residual, amount_untaxed_signed, amount_total_signed,
        amount_residual_signed, fiscal_position_id, invoice_user_id, state,
        invoice_payment_state, invoice_date, invoice_date_due,
        invoice_payment_ref, invoice_sent, invoice_origin,
        ...
        ai.amount_untaxed_signed, COALESCE(ai.amount_total_signed,
        ai.amount_total_company_signed), ai.residual_signed,
    ...
    openupgrade.logged_query(
        ...
        invoice_payment_term_id, invoice_partner_bank_id, invoice_incoterm_id,
        invoice_source_email, invoice_partner_display_name,
        invoice_cash_rounding_id, old_invoice_id, create_uid, create_date,
        ...
        amount_total_signed, residual_signed, fiscal_position_id, user_id,
    ...
    ids = env.cr.fetchall()
    if ids:
        _move_model_in_data(env, ids, 'account.invoice', 'account.move')
    openupgrade.logged_query(
        env.cr, """
...
def migration_voucher_moves(env):
    ...
    openupgrade.logged_query(
        env.cr, """
        INSERT INTO account_move (message_main_attachment_id, name, date, ref,
        narration, type, journal_id, company_id, currency_id, partner_id,
        amount_tax, amount_total, state, invoice_payment_state, invoice_date,
        invoice_date_due, invoice_payment_ref, old_voucher_id,
        create_uid, create_date, write_uid, write_date)
        SELECT message_main_attachment_id, COALESCE(number, '/'), date, reference, narration,
        CASE WHEN voucher_type = 'purchase' THEN 'in_receipt'
        ELSE 'out_receipt' END, journal_id, company_id, currency_id,
        partner_id, tax_amount, amount, state, 'not_paid', account_date,
        date_due, name, id, create_uid, create_date, write_uid, write_date
        FROM account_voucher av
    ...
    if ids:
        _move_model_in_data(
            env, ids, 'account.voucher', 'account.move')
    openupgrade.logged_query(
        env.cr, """
        INSERT INTO account_move_line (company_id, account_id,
